...
struct mount* rootfs;
char cwdpath[PATHNAME_MAX];
file_t* kfd[MAX_NUM_OF_ENTRY];
int fd_count=0;
...
int register_filesystem(struct filesystem* fs)
{
  // register the file system to the kernel.
    ...
    {
        if(strequ(fs->name, fs_list[i]->name))
            return -1;
    }
...
int vfs_mount(char* target, char* filesystem)
{
    if(strequ(target, "/"))
    {
        ...
        rootfs->fs = malloc(sizeof(filesystem_t));
        rootfs->fs->name = malloc(strlen(filesystem)+1);
        strcpy(filesystem, rootfs->fs->name);
        rootfs->fs->setup_mount = tmpfs_mount;
        register_filesystem(rootfs->fs);
        return rootfs->fs->setup_mount(rootfs->fs, rootfs);
    }
    else if(strequ(target, "/initramfs"))
    {
        ...
        dir_node->mount = malloc(sizeof(mount_t));
        dir_node->mount->root = dir_node;
        dir_node->mount->fs = malloc(sizeof(filesystem_t));
        dir_node->mount->fs->name = malloc(strlen(filesystem)+1);
        strcpy(filesystem, dir_node->mount->fs->name);
    ...
    else if(strequ(target, "/boot"))
    {
        ...
        dir_node->mount->fs->name = malloc(strlen(filesystem)+1);
        strcpy(filesystem, dir_node->mount->fs->name);
        dir_node->mount->fs->setup_mount = fat32_mount;
        register_filesystem(dir_node->mount->fs);
        return dir_node->mount->fs->setup_mount(dir_node->mount->fs, dir_node->mount);
    }
    else
    {
        ...
        if(register_filesystem(dir_node->mount->fs)==-1)
        {
            ...
            dir_node->internal = malloc(sizeof(node_info_t));
            dir_node->internal->name = malloc(strlen(dir_name)+1);
            strcpy(dir_name, dir_node->internal->name);

            dir_node->internal->type = DIR;
            dir_node->internal->size = 0;
            dir_node->internal->entry = malloc(sizeof(vnode_t*)*MAX_NUM_OF_ENTRY);
...
void handle_path(char* rela, char* abso)
{
    ...
    while(rela[i]!='\0')
    {
        ...
        else
        {
            ...
            if(strequ(next_component, "."));
            else if(strequ(next_component, ".."))
                dirname(abso, abso);
            else
            {
