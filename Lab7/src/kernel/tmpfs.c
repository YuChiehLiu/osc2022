...
{
    mount->root = malloc(sizeof(vnode_t));
    ...
    mount->root->internal = malloc(sizeof(node_info_t));
    mount->root->internal->name = malloc(strlen("/")+1);
    strcpy("/", mount->root->internal->name);

    mount->root->internal->type = DIR;
    mount->root->internal->size = 0;
    mount->root->internal->entry=malloc(sizeof(vnode_t*)*MAX_NUM_OF_ENTRY);
...
int tmpfs_lookup(struct vnode* dir_node, struct vnode** target, char* component_name)
{
    ...
    {
        if(strequ(dir_node->internal->entry[i]->internal->name, component_name))
        {
...
int tmpfs_create(struct vnode* dir_node, struct vnode** target, char* component_name)
{
    ...
    *target = malloc(sizeof(vnode_t));
    (*target)->mount = NULL;
    (*target)->v_ops = dir_node->v_ops;
    (*target)->f_ops = dir_node->f_ops;

    (*target)->internal = malloc(sizeof(node_info_t));
    (*target)->internal->name = malloc(sizeof(COMPONENT_NAME_MAX));
    strcpy(component_name, (*target)->internal->name);
    (*target)->internal->type = FILE;
    (*target)->internal->size = 0;
    (*target)->internal->entry = NULL; // mkdir will malloc()
    (*target)->internal->data = NULL; // open will malloc()
...
int tmpfs_mkdir(struct vnode* dir_node, struct vnode** target, char* component_name)
{
    ...
    if(ret == 0)
    {
        ...
        (*target)->internal->entry = malloc(sizeof(vnode_t*) * MAX_NUM_OF_ENTRY);
        return 0;
    }
    else
        return -1;
...
int tmpfs_open(struct vnode* file_node, struct file** target)
{
    if(file_node->internal->data == NULL)
        file_node->internal->data = malloc(sizeof(char) * TMPFS_FILE_SIZE_MAX);
