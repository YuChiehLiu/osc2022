...
void L1_timer_handler(long cntpct_el0, long cntfrq_el0)
{
    ...
    if (!is_empty())
    {
        ...
        uart_puts("Message : ");
        uart_puts(node[timer_front].message);
        uart_puts("\n");

        timer_front = (timer_front + 1) % TIMER_QUEUE_SIZE;
        if (!is_empty())
        {
            find_min();
            asm volatile(
...
{
    int syscall_no;
    trapframe_t *curr_trapframe = (trapframe_t *)trapframe_addr;
    ...
    else if (syscall_no == 4)
    {
        ...
        int ret = fork();
        curr_trapframe = (trapframe_t *)get_current()->trapframe;
        curr_trapframe->x[0] = ret;
    }
