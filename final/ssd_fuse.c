...
union pca_rule
{
    ...
    struct
    {
        unsigned int lba : 16; // # of used page
        unsigned int nand: 16; // this  pca is ong which nand(block)
    } fields;
};
...
static int ftl_write(const char* buf, size_t lba_rnage, size_t lba)
{
    ...
    for(int i=0 ; i<lba_rnage ; i++)
    {
        new_pca.pca = get_next_pca();

        nand_write(buf+i*512, new_pca.pca);
        if(L2P[lba+i]!=INVALID_PCA)
        {
            PCA_RULE old_pca;
            old_pca.pca = L2P[lba+i];
            P2L[old_pca.fields.nand*10 + old_pca.fields.lba] = INVALID_LBA;
        }
        P2L[new_pca.fields.nand*10 + new_pca.fields.lba] = lba+i;
        L2P[lba+i] = new_pca.pca;
    }
...
static int gc()
{
    ...
    int free_block_list[PHYSICAL_NAND_NUM];
    ...
    if(3<free_block_number && free_block_number<10)
    {
        for(int i=1 ; i<PHYSICAL_NAND_NUM ; i++)
        {
            ...
            int idx; 
            if( i==0 )
                idx = (curr_pca.fields.nand + i) % PHYSICAL_NAND_NUM;
            else 
                idx = (idx + 1)%PHYSICAL_NAND_NUM;

            if(valid_count[idx] == FREE_BLOCK)
                continue;

            for(int j=0 ; j<free_block_count ; j++)
                if(idx==free_block_list[i])
                    continue;
            
            int invalid_count = 0;
            for(int j=0 ; j<PAGE_PER_BLOCK ; j++)
            {
                if(P2L[idx*10+j]==INVALID_LBA)
                    invalid_count++;
            }
            ...
            if(invalid_count>=7)
            {
                ...
                for(int j=0 ; j<PAGE_PER_BLOCK ; j++)
                {
                    if(P2L[idx*10+j]!=INVALID_LBA)
                    {
                        ...
                        ftl_read(buf, P2L[idx*10+j]);
                        ftl_write(buf, 1, P2L[idx*10+j]);
                    }
                }
                nand_erase(idx);
                free_block_number++;
...
static int ssd_do_write(const char* buf, size_t size, off_t offset)
{
    ...
    for (idx = 0; idx < tmp_lba_range; idx++)
    {
        ...
        if(idx == 0)
            ...
            process_size = remain_size < 512 ? remain_size : 512;

        memcpy(tmp_buf+(offset%512)+curr_size, buf+curr_size, process_size);
