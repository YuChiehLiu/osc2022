...
int config_pf(int size_req)
{
    ...
    int RM_index = is_include_RM(pf[cfg_page].addr, pf[cfg_page].addr + MIN_PAGE_SIZE * pow(2, pf[cfg_page].allord) - 1);
    if (RM_index)
    {
...
void memory_init()
{
    ...
    for (int i = 0; i < TOTAL_PAGE_FRAME; i++)
    {
        if (!is_include_RM(pf[i].addr, pf[i].addr + MIN_PAGE_SIZE * -1))
            free_pf(i);
    }
}
